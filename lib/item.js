	...
	if (!this.loaded || this.filepath != filepath) {
		...
		fs.readFile(this.filepath, {encoding: 'utf8'}, function(err, rawContent) {
			...
			if (this.meta('template')) {
				this.loadPathList.add(path.join(config.get('THEME_ROOT'), this.meta('template')), 4);
			}
			if (this.type.config.template) {
				this.loadPathList.add(path.join(config.get('THEME_ROOT'), this.type.config.template), 3);
			}
			this.loadPathList.add(path.join(config.get('THEME_ROOT'), this.type.name + '.html'), 2);
			this.loadPathList.add(path.join(config.get('THEME_ROOT'), 'index.html'), 1);
