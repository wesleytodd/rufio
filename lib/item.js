...
Item.prototype.loadFile = function(filepath, done) {
	...
	if (!this.loaded || this.filepath != filepath) {
		...
		fs.readFile(this.filepath, {encoding: 'utf8'}, function(err, rawContent) {
			...
			this.loadPathList = new this.rufio.util.LoadPathList();
			if (this.meta('template')) {
				this.loadPathList.add(path.join(this.rufio.config.get('THEME_ROOT'), this.meta('template')), 4);
			}
			if (this.type.config.template) {
				this.loadPathList.add(path.join(this.rufio.config.get('THEME_ROOT'), this.type.config.template), 3);
			}
			this.loadPathList.add(path.join(this.rufio.config.get('THEME_ROOT'), this.type.name + '.html'), 2);
			this.loadPathList.add(path.join(this.rufio.config.get('THEME_ROOT'), 'index.html'), 1);
...
Item.prototype.render = function(done) {
	...
	var renderTemplate = function(tmpl) {
		...
		contentFilters.forEach(function(f) {
			renderData.content = this.rufio.filters.apply(f, renderData.content, renderData);
		}.bind(this));
...
Item.prototype.write = function(done) {
	...
	if (this.rufio.config.get('ENVIRONMENT') == 'dev' || this.meta('status') == 'Published') {
		...
		var writeFile = function(content) {
			...
			var writePath = path.join(this.rufio.config.get('BUILD_ROOT'), this.meta('permalink'));
...
Item.prototype.lookupTemplate = function(done) {
	...
	if (!this.loaded) {
		this.rufio.logger.error('Item must be loaded before calling lookupTemplate');
	...
	if (!this.templatePath) {
		...
		this.loadPathList.load(function(filepath, done) {
			...
			if (typeof templatePath === 'undefined') {
				this.rufio.logger.error('Error looking up tempalte');
...
Item.prototype.loadTemplate = function(done) {
	...
	if (!this.templatePath) {
		this.lookupTemplate(function(err, templatePath) {
			if (err) {
				this.rufio.logger.error('Error looking up template.', err);
				return done(err);
			}
		...
		// Log it
		this.rufio.logger.info('Loading template for ' + this.metadata.title + ': ' + this.templatePath);

		// Read the file
		fs.readFile(this.templatePath, {encoding: 'utf8'}, function(err, content) {
			// return error
			if (err) {
				this.rufio.logger.error('Error reading file: ', err);
				return done(err);
			}
